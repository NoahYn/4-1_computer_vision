...
void getGaussFilter(double(*filter)[5], int size, double std) {
	double sum = 0;
	...
	for (int i = 0; i < size; i++) {
		for (int j = 0; j < size; j++) {
			...
			filter[i][j] = exp(-(x * x + y * y) / (2 * std * std));
			sum += filter[i][j];
		}
	}
	for (int i = 0; i < size; i++)
...
int main() {
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			...
			gaussian[i][j] = origin[i][j] + gaussnoise;
	...
	for (int i = 0; i < W * H * 0.3; i++) {
		int randi = rand() % H;
		int randj = rand() % W;
		saltpepper[randi][randj] = (rand() % 2) * 255;
	}
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			...
			for (int dir = 0; dir < 9; dir++) {
				...
				sum += gaussian[x][y] * filter[dx3[dir]][dy3[dir]];
			}
			fputc((unsigned char)sum, GG3);
			double diff = origin[i][j] - (unsigned char)sum;
			SSE += diff * diff;
		}
	}
	MSE = SSE / (W * H);
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			...
			for (int dir = 0; dir < 9; dir++) {
				...
				int y = j + dy3[dir] - 1;
				if (x < 0 || x >= H || y < 0 || y >= W) continue; // zero padding : nothing to add
				sum += saltpepper[x][y] * filter[dx3[dir]][dy3[dir]];
			}
			fputc((unsigned char)sum, SG3);
			double diff = origin[i][j] - (unsigned char)sum;
			SSE += diff * diff;
		}
	}
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			...
			for (int dir = 0; dir < 25; dir++) {
				...
				int y = j + dy5[dir] - 2;
				if (x < 0 || x >= H || y < 0 || y >= W) continue; // zero padding : nothing to add
				sum += gaussian[x][y] * filter[dx5[dir]][dy5[dir]];
			}
			fputc((unsigned char)sum, GG5);
			double diff = origin[i][j] - (unsigned char)sum;
			SSE += diff * diff;
		}
	}
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			...
			for (int dir = 0; dir < 9; dir++) {
				...
				int y = j + dy5[dir] - 2;
				if (x < 0 || x >= H || y < 0 || y >= W) continue; // zero padding : nothing to add
				sum += saltpepper[x][y] * filter[dx5[dir]][dy5[dir]];
			}
			fputc((unsigned char)sum, SG5);
			double diff = origin[i][j] - (unsigned char)sum;
			SSE += diff * diff;
		}
	}
	...
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			for (int dir = 0; dir < 25; dir++) {
				int x = i + dx5[dir] - 2;
				int y = j + dy5[dir] - 2;
				if (x < 0 || x >= H || y < 0 || y >= W) filter2[dir] = 0; // zero padding : nothing to add
				filter2[dir] = saltpepper[x][y];
			}
			qsort(filter2, 25, sizeof(unsigned char), cmp);
			double diff = origin[i][j] - filter2[12];
